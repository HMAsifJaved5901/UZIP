<form action="{{isset($route)?$route:route('expense.store')}}" method="POST" >
    {{csrf_field()}}
    <input type="hidden" name="_method" value="{{isset($method)?$method:'POST'}}"/>
    <div class="form-group">
    <label for="expense_date">Expense Date</label>
    <div class="input-group">
        <input type="date" class="form-control {{ $errors->has('expense_date') ? ' is-invalid' : '' }}" name="expense_date" id="expense_date"
               value="{{old('expense_date',$model->expense_date)}}"
               placeholder="" required="required" >
        <div class="input-group-addon">
            <label for="expense_date" class="fa fa-calendar">
            </label>
        </div>
    </div>
