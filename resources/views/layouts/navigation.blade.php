<aside id="layout-menu" class="layout-menu menu-vertical menu bg-menu-theme">
    <div class="app-brand demo">
        <a href="{{url('/')}}" class="app-brand-link">
              <span class="app-brand-logo demo">
                <svg width="32" height="22" viewBox="0 0 32 22" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path
                          fill-rule="evenodd"
                          clip-rule="evenodd"
                          d="M0.00172773 0V6.85398C0.00172773 6.85398 -0.133178 9.01207 1.98092 10.8388L13.6912 21.9964L19.7809 21.9181L18.8042 9.88248L16.4951 7.17289L9.23799 0H0.00172773Z"
                          fill="#7367F0"
                  />
                  <path
                          opacity="0.06"
                          fill-rule="evenodd"
                          clip-rule="evenodd"
                          d="M7.69824 16.4364L12.5199 3.23696L16.5541 7.25596L7.69824 16.4364Z"
                          fill="#161616"
                  />
                  <path
                          opacity="0.06"
                          fill-rule="evenodd"
                          clip-rule="evenodd"
                          d="M8.07751 15.9175L13.9419 4.63989L16.5849 7.28475L8.07751 15.9175Z"
                          fill="#161616"
                  />
                  <path
                          fill-rule="evenodd"
                          clip-rule="evenodd"
                          d="M7.77295 16.3566L23.6563 0H32V6.88383C32 6.88383 31.8262 9.17836 30.6591 10.4057L19.7824 22H13.6938L7.77295 16.3566Z"
                          fill="#7367F0"
                  />
                </svg>
              </span>
            <span class="app-brand-text demo menu-text fw-bold">UZIP</span>
        </a>

        <a href="javascript:void(0);" class="layout-menu-toggle menu-link text-large ms-auto">
            <i class="ti menu-toggle-icon d-none d-xl-block ti-sm align-middle"></i>
            <i class="ti ti-x d-block d-xl-none ti-sm align-middle"></i>
        </a>
    </div>

    <div class="menu-inner-shadow"></div>

    <ul class="menu-inner py-1">
        <!-- Dashboards -->
        <li class="menu-item {{ Request::is('/') ? 'active' : '' }}">
            <a href="{{ url('/') }}" class="menu-link">
                <i class="menu-icon tf-icons fa fa-dashboard"></i>
                <div data-i18n="Dashboards">Dashboards</div>
                <div class="badge bg-label-primary rounded-pill ms-auto"></div>
            </a>
        </li>

        <!-- Administration -->
        <li class="menu-header small text-uppercase">
            <span class="menu-header-text">Configuration</span>
        </li>

        <li class="menu-item {{ Request::is('company*') ? 'active' : '' }}">
            <a href="{{ route('company.index') }}" class="menu-link">
                <i class="menu-icon  tf-icons fa-regular fa-building"></i>
                <div data-i18n="Companies"> Companies</div>
            </a>
        </li>
        <li class="menu-item {{ Request::is('station*') ? 'active' : '' }}">
            <a href="{{ route('station.index') }}" class="menu-link">
                <i class="menu-icon  tf-icons fas fa-gas-pump"></i>
                <div data-i18n="Stations"> Stations</div>
            </a>
        </li>
        <li class="menu-item {{ Request::is('service*') ? 'active' : '' }}">
            <a href="{{route('service.index')}}" class="menu-link">
                <i class="menu-icon tf-icons fa fa-wrench"></i>
                <div data-i18n="Services">Services</div>
            </a>
        </li>

        <li class="menu-item {{ Request::is('profile*') ? 'active' : '' }}">
            <a href="{{ route('profile.index') }}" class="menu-link">
                <i class="menu-icon tf-icons ti ti-users"></i>
                <div data-i18n="Users">Users</div>
            </a>
        </li>
        <li class="menu-item {{ Request::is('roles*') ? 'active' : '' }}">
            <a href="{{'roles'}}" class="menu-link">
                <i class="menu-icon tf-icons fa fa-tasks"></i>
                <div data-i18n="Roles">Roles</div>
            </a>
        </li>
        <li class="menu-item {{ Request::is('permissions*') ? 'active' : '' }}">
            <a href="{{'permissions'}}" class="menu-link">
                <i class="menu-icon tf-icons fa fa-lock"></i>
                <div data-i18n="Permission">Permission</div>
            </a>
        </li>

        {{--<li class="menu-item {{ Request::is('roles*')||Request::is('permissions') ? 'open' : '' }}">--}}
            {{--<a href="javascript:void(0);" class="menu-link menu-toggle">--}}
                {{--<i class="menu-icon tf-icons fa fa-tasks"></i>--}}
                {{--<div data-i18n="Roles & Permissions">Roles & Permissions</div>--}}
            {{--</a>--}}
            {{--<ul class="menu-sub">--}}
                {{--<li class="menu-item {{ Request::is('roles*') ? 'active' : '' }}">--}}
                    {{--<a href="{{'roles'}}" class="menu-link">--}}
                        {{--<div data-i18n="Roles">Roles</div>--}}
                    {{--</a>--}}
                {{--</li>--}}
                {{--<li class="menu-item {{ Request::is('permissions*') ? 'active' : '' }}">--}}
                    {{--<a href="{{'permissions'}}" class="menu-link">--}}
                        {{--<div data-i18n="Permission">Permission</div>--}}
                    {{--</a>--}}
                {{--</li>--}}
            {{--</ul>--}}
        {{--</li>--}}
        {{--<li class="menu-item {{ Request::is('configuration*') || Request::is('service*') || Request::is('company*') || Request::is('lookup*') || Request::is('category*') || Request::is('station*') ? 'open' : '' }}">--}}
            {{--<a href="javascript:void(0);" class="menu-link menu-toggle">--}}
                {{--<i class="menu-icon tf-icons fa fa-gear"></i>--}}
                {{--<div data-i18n="Configuration">Configuration</div>--}}
            {{--</a>--}}
            {{--<ul class="menu-sub">--}}
              {{----}}
            {{--</ul>--}}
        {{--</li>--}}

        <li class="menu-item {{ Request::is('configuration*') ? 'active' : '' }}">
            <a href="{{route('configuration.index')}}" class="menu-link">
                <i class="menu-icon tf-icons fa fa-gear"></i>
                <div data-i18n="General Settings">General Settings</div>
            </a>
        </li>
        <li class="menu-item {{ Request::is('lookup/transaction') ? 'active' : '' }}">
            <a href="{{route('lookup.transaction.list')}}" class="menu-link">
                <i class="menu-icon tf-icons fa fa-gear"></i>
                <div data-i18n="Transaction Types">Transaction Types</div>
            </a>
        </li>
        <li class="menu-item {{ Request::is('lookup/income') ? 'active' : '' }}">
            <a href="{{route('lookup.income.list')}}" class="menu-link">
                <i class="menu-icon tf-icons fa fa-gear"></i>
                <div data-i18n="Income Types">Income Types</div>
            </a>
        </li>
        <li class="menu-item {{ Request::is('lookup/expense') ? 'active' : '' }}">
            <a href="{{route('lookup.expense.list')}}" class="menu-link">
                <i class="menu-icon tf-icons fa fa-gear"></i>
                <div data-i18n="Expense Types">Expense Types</div>
            </a>
        </li>
        <!-- Administration -->
        <li class="menu-header small text-uppercase">
            <span class="menu-header-text">Operations</span>
        </li>

        <li class="menu-item {{ Request::is('income*') ? 'active' : '' }}">
            <a href="{{ route('income.index') }}" class="menu-link">
                <i class="menu-icon tf-icons fas fa-money-bill-wave-alt"></i>
                <div data-i18n="Income">Income</div>
            </a>
        </li>
        <li class="menu-item {{ Request::is('expense*') ? 'active' : '' }}">
            <a href="{{ route('expense.index') }}" class="menu-link">
                <i class="menu-icon tf-icons fas fa-money-bill-wave-alt"></i>
                <div data-i18n="Expense">Expense</div>
            </a>
        </li>
    </ul>
</aside>
